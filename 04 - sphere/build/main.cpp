...
bool hit_sphere(const point3& center, double radius, const ray& r){
   ...
   *  c = (C - Q) dot (C - Q) - r^2
   *  t = (-b +- sqrt(b^2 - 4ac))/(2a)
   *  1 root: ray hit sphere one times
   *  2 root: ray hit sphere two times
   */ 
  vec3 ori_pos = center - r.origin();
  auto a = dot(r.direction(), r.direction());
  auto b = -2.0 * dot(r.direction(), ori_pos);
  auto c = dot(ori_pos, ori_pos) - radius*radius;
  auto discrimiant = b*b - 4*a*c;
